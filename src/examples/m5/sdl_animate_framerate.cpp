...
/// SDL Programming
////////////////////////////////////////////////////////////////////////////////
/// Example of sprite animation with constant frame rate, despite 
/// the rendering speed.
...
int main( int argc, char **argv )
{
  ...
  float fCumulativeTimeInSeconds = 0.0f;
  Uint32 iPreviousTime = SDL_GetTicks();   
  float fAnimFramesPerSecond = 5.0f;
  float fFrameDisplayTimeInSec = 1.0f/fAnimFramesPerSecond;
  ...
  while ( bRunning )
  {
    ...
    if ( bAnimating ) 
    {
      Uint32 iCurrTime = SDL_GetTicks(); 
      fCumulativeTimeInSeconds += ((iCurrTime - iPreviousTime ) * 0.001f);
      
      if ( fCumulativeTimeInSeconds > fFrameDisplayTimeInSec )
      {
	int incr = static_cast<int>(fCumulativeTimeInSeconds / 
				    fFrameDisplayTimeInSec );
	fCumulativeTimeInSeconds -= (incr * fFrameDisplayTimeInSec);
	frame+=incr;
      }
      iPreviousTime = iCurrTime;
    }
    ...
    // Change this to 500 if you want to see how frames are dropped.
    SDL_Delay(500);
